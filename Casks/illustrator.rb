cask :v1 => 'illustrator' do
  ...
  installer :script => "#{Utils.name}/Install.app/Contents/MacOS/Install",
            :args => ['--mode=silent', "--deploymentFile=/usr/local/Caskroom/illustrator/18.0.0/#{Utils.name}/Deployment/#{Utils.install_locale}_Deployment.xml"]

  uninstall :script => "#{Utils.name}/Install.app/Contents/MacOS/Install",
            :args => ['--mode=silent', "--action=uninstall", "--deploymentFile=/usr/local/Caskroom/illustrator/18.0.0/#{Utils.name}/Deployment/#{Utils.install_locale}_Deployment.xml"]
